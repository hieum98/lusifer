...
from lusifer.eval.constants import CROSS_LINGUAL_INSTRUCTION, MTEB_DS_TO_PROMPT, QUICK_EVAL, LANG_TO_CODES, MULTILINGUAL_DS_TO_PROMPT
from lusifer.eval.wrapped_hf_model import WrappedHFModel
try:
    from lusifer.models.lusifer import WrappedLusifer, Lusifer
except:
    pass
...
if __name__=='__main__':
    ...
    parser.add_argument(
        "--is_lusifer", action='store_true', help="Whether the model is Lusifer"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval_crosslingual", action='store_true', help="Whether to evaluate crosslingual",
    )
    parser.add_argument(
    ...
    elif args.is_lusifer:
        model = Lusifer.from_pretrained(args.model_name_or_path)
    else:
    ...
    if args.eval_crosslingual:
        print("Evaluating on all languages for crosslingual evaluation")
        langs = ['en', 'es', 'ru', 'fr', 'vi', 'fa', 'id', 'ar', 'fi', 'ko', 'hi', 'bn', 'te', 'sw']
        codes = []
        for lang in langs:
            codes.extend(LANG_TO_CODES[lang])
        
        if args.dataname is None:
            datanames = CROSS_LINGUAL_INSTRUCTION.keys()
        else:
            datanames = [args.dataname]
        for dataname in datanames:
            output_path = os.path.join(args.output_folder, f'crosslingual_{dataname}_results.json')
            instruction = CROSS_LINGUAL_INSTRUCTION.get(dataname, None)
            all_results = {}
            if instruction is None:
                print(f"Could not find the instruction for {dataname}")
            else:
                results = eval_mteb_dataset(
                    dataset_name=dataname,
                    instruction=instruction,
                    langs=codes,
                    model=model,
                    output_folder=f"{args.output_folder}/crosslingual",
                    batch_size=batch_size,
                    max_length=args.max_length
                )
                if results is not None:
                    # Compute the average of the results for all languages, ignore the ones that are None or NaN in numpy
                    avg = np.nanmean([v for v in results.values() if not np.isnan(v)])
                    results['avg'] = avg
                    all_results[f'{dataname}'] = results

            with open(os.path.join(args.output_folder, f'crosslingual_{dataname}_results.json'), 'w', encoding='utf-8') as f:
                json.dump(all_results, f, indent=4)
